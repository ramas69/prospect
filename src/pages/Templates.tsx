...
export default function Templates() {
  ...
  const [modèles, setTemplates] = useState<Template[]>([]);
  ...
  const loadTemplates = async () => {
    ...
    const { data: modèlesData } = await supabase
      .from('modèles')
      .select('*')
      // .eq('user_id', profile.id) // Enable shared modèles
      .order('created_at', { ascending: false });

    setTemplates(modèlesData || []);
    setLoading(false);
  };
  ...
  const handleCreateTemplate = async (e: React.FormEvent) => {
    ...
    if (editingTemplate) {
      ...
      const { error: insertError } = await supabase.from('modèles').insert({
        user_id: profile.id,
        ...formData,
      });
      error = insertError;
    ...
    if (error) {
      showNotification('error', `Erreur lors de la ${editingTemplate ? 'modification' : 'création'} du modèle`);
      return;
    }
  ...
  const handleEdit = (modèle: Template) => {
    setEditingTemplate(modèle);
    setFormData({
      name: modèle.name,
      sector: modèle.sector,
      location: modèle.location || '',
      limit_results: modèle.limit_results,
    });
  ...
  const toggleFavorite = async (modèle: Template) => {
    const { error } = await supabase
      .from('modèles')
      .update({ is_favorite: !modèle.is_favorite })
      .eq('id', modèle.id);
    ...
    showNotification(
      'success',
      modèle.is_favorite ? 'Retiré des favoris' : 'Ajouté aux favoris'
    );
    loadTemplates();
  };
  ...
  const deleteTemplate = async (id: string) => {
    showConfirm(
      ...
      async () => {
        const { error } = await supabase.from('modèles').delete().eq('id', id);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            {modèles.length} modèle{modèles.length > 1 ? 's' : ''} sauvegardé{modèles.length > 1 ? 's' : ''}
          </p>
        </div>
        <button
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">{modèle.sector}</p>
                </div>
                <button
                  onClick={() => toggleFavorite(modèle)}
                  className={`p-2 rounded-lg transition-colors ${modèle.is_favorite
                    ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400'
                    : 'bg-gray-100 dark:bg-gray-700 text-gray-400 hover:text-yellow-500'
                    }`}
                >
                  <Star className={`w-5 h-5 ${modèle.is_favorite ? 'fill-current' : ''}`} />
                </button>
              </div>
              ...
              <div className="flex items-center gap-4 mb-4 text-sm text-gray-600 dark:text-gray-400">
                <span>Limite: {modèle.limit_results}</span>
                <span>•</span>
                <span>Utilisé {modèle.use_count} fois</span>
              </div>
              ...
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  onClick={() => deleteTemplate(modèle.id)}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-3xl p-8 max-w-md w-full">
            ...
            <form onSubmit={handleCreateTemplate} className="space-y-4">
              ...
              >
                {editingTemplate ? 'Modifier le modèle' : 'Créer le modèle'}
              </button>
